...
void exhibitStatusMsg(Character *handledCharacters[CHARACTERNUM]) {//動かせるキャラクタのステータス情報を表示する。
...
void exhibitScreen(int markX, int markY ,bool attention ,Grid board[][FIELDSIZE], Character *handledCharacters[CHARACTERNUM]) {//ペンギンを描画（ステータスと向きからペンギンの適切な画像のハンドルを入手し格納してから描画）
	int h;//ハンドル格納用
	string turn = "";
	bool HPexhibitOrNot;
