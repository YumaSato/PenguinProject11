...
bool Character::walk( Grid** board, Emperor* handledCharacters) {//歩く。盤面サイズ(size)を受け取る
	...
	int checkX = 0;
	int checkY = 0;
	int staminaNeed = staminaRecoverAbility + 3;//歩ける距離
	...
	actionMsg = std::to_string(staminaNeed)+"マスまで歩けるよ。\nEsc:移動終了　SHIFT:斜めサポート";
	...
	while (distance < staminaRecoverAbility + 3) {//各歩行の入力待機
